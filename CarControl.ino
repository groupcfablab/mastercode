...
void setup()
{
	...
	//motor_right.setup(20 * k, 150 * k, 0.3*k, pwmpin__right, directionpin_right, interruptpin_right, magnets_on_circle, min_pwm_for_motor, max_rated_rpm);
        car.setup(20 * k, 150 * k, 0.3*k,pwmpin__left,pwmpin__right,directionpin_left,directionpin_right,interruptpin_left,interruptpin_right,magnets_on_circle,min_pwm_for_motor,max_rated_rpm);
}

void loop()
{
	//motor_left.drive(80, false);
	//motor_right.drive(140, false);
        while(car.drive_turn(180,100,200))
        {
          car.drive_turn(180,0,2000);
        }
        //while(!car.drive_turn(180,0,2000))
}
